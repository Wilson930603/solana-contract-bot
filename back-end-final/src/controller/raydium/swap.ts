...
// Add the import statement for sendTipToJito right here
import { sendTipToJito } from './jitoIntegration';
...
class RaydiumSwap {
  ...
  constructor(RPC_URL: string, WALLET_PRIVATE_KEY: string) {
    this.connection = new Connection(RPC_URL, { commitment: "confirmed", confirmTransactionInitialTimeout: 10000 });
  ...
  async loadPoolKeys(liquidityFile: string) {
    const liquidityJson = await axios.get(liquidityFile);
    const allPoolKeysJson = [
      ...(liquidityJson.data?.official ?? []),
      ...(liquidityJson.data?.unOfficial ?? []),
    ];
  ...
  async getPoolKeys(poolId: string) {
    ...
    const info = (await this.connection.getAccountInfo(poolPubkey)) as any;
   ...
   * @returns {LiquidityPoolKeys} The liquidity pool keys if found, otherwise null.
   */
  ...
  async getSwapTransaction(
    ...
    // console.log(amount, amountIn.toExact(), minAmountOut.toExact())

    const userTokenAccounts = await this.getOwnerTokenAccounts();

    // Before constructing the swap transaction
// try {
  // Assuming `getJitoTipAccount` is a method that randomly selects a Jito tip account
 
 
 // const selectedTipAccount = this.getJitoTipAccount(); 
 // await sendTipToJito(this.wallet, selectedTipAccount, 0.01, this.connection);
//} catch (error) {
 // console.error("Error sending tip to Jito:", error);
 // throw error; // Or handle the error based on your application's logic
//}

// Now proceed with creating the swap transaction
...
const swapTransaction = await Liquidity.makeSwapInstructionSimple({
  ...
  async calcAmountOut(
    ...
    const slippage = new Percent(200, 100); // 5% slippage
